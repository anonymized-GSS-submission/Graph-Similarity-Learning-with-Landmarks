...
# def sendmessage(title = 'title',content = 'test',template='html'):
#     url = f'http://pushplus.hxtrip.com/send?token=7dd3e416a60d4d7388a56eb372521788&title={title}&content={content}&template={template}'
#     requests.get(url)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # sendmessage(title = 'done',content = 'done')
        # message = makehtml(
        ...
        # sendmessage(title='alpha GraphSimilarity {} - {} result is coming!'.format(args.model,args.datasets), content=message)
    except Exception as e:
        # sendmessage(title='alpha GraphSimilarity {} - {} is error!'.format(args.model,args.datasets))
        raise e