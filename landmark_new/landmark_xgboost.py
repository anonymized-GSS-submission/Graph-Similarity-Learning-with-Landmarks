...
def main():
    ...
    p_mat_train,gt_train = GetKernel(train_datasets,train_datasets,model)
    p_mat_val,gt_val = GetKernel(train_datasets,vaild_datasets,model)
    p_mat_test,gt_test = GetKernel(train_datasets,test_datasets,model)

    # torch.save(p_mat_train,f"./landmark_new/temp/{datasets}_p_mat_train.pt")
    # torch.save(gt_train,f"./landmark_new/temp/{datasets}_gt_train.pt")
    # torch.save(p_mat_val,f"./landmark_new/temp/{datasets}_p_mat_val.pt")
    # torch.save(gt_val,f"./landmark_new/temp/{datasets}_gt_val.pt")
    # torch.save(p_mat_test,f"./landmark_new/temp/{datasets}_p_mat_test.pt")
    # torch.save(gt_test,f"./landmark_new/temp/{datasets}_gt_test.pt")
    ...
    # bst_model.predict(dtest)
# for eta in [0.01*i for i in range(10,20)]:
    param1 = {
        "AIDS700nef":{"objective":"reg:squarederror","verbosity":1#,"booster":'dart'
            ,"subsample":1
            ,"max_depth":7
            ,"eta":0.14#0.14
            ,"gamma":0
            ...
            ,"colsample_bylevel":1#
            ,"colsample_bynode":1},
        "IMDBMulti":{"objective":"reg:squarederror","verbosity":1#,"booster":'dart'
            ,"subsample":1#0.8
            ,"max_depth":24
            ,"eta":0.08#0.14，0.05
            ,"gamma":0
            ...
            ,"alpha":0
            ,"colsample_bytree":1
            ,"colsample_bylevel":1##,'tree_method':'gpu_hist','gpu_id':0,"predictor": "gpu_predictor"}
            ,"colsample_bynode":1},
        "LINUX":{"objective":"reg:squarederror","verbosity":1#,"booster":'dart'
            ,"subsample":1
            ,"max_depth":7
            ,"eta":0.14#0.14
            ,"gamma":0
    ...
    # param1 = {"objective":"reg:squarederror","verbosity":1#,"booster":'dart'
    #         ,"subsample":1
    #         ,"max_depth":7
    #         ,"eta":0.14#0.14
    #         ,"gamma":0
    ...
    #         ,"colsample_bytree":1
    #         ,"colsample_bylevel":1#
    #         ,"colsample_bynode":1}#,'tree_method':'gpu_hist','gpu_id':0,"predictor": "gpu_predictor"}

    # interaction_constraints = ['[i,i+len(landmark_id)],' for i in range(len(landmark_id))]
    ...
    num_round = 10#3000
    start = time.time()
    best_model =xgboost.train(param1[datasets], dtrain, num_round,evallist)#,eval_metric =evallist
